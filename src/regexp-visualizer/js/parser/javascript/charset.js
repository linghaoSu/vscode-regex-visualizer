...
import { map, uniqBy } from 'lodash-unified';
...
export default {
  ...
  async _render() {
    ...
    await Promise.all(map(this.elements,
      async part => await part.render(this.partContainer.group()),
    ));
  ...
  setup() {
    ...
    this.elements = uniqBy(this.properties.parts.elements,
