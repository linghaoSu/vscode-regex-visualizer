...
// modules.
import { defaults, extend, flatten, forOwn } from 'lodash-unified';
...
export default class Node {
  ...
  set module(mod) {
    ...
    forOwn(this.definedProperties || {}, (methods, name) => {
  ...
  getBBox() {
    return extend(util.normalizeBBox(this.container.getBBox?.()), this.anchor);
  }
  ...
  async render(container) {
    ...
    if (this.proxy) {
      ...
      return await this.proxy.render(this.container);
    } else {
  ...
  async renderLabel(text) {
    ...
    const label = group.text(0, 0, flatten([text]));
  ...
  async renderLabeledBox(text, content, options) {
    const label = this.container.text(0, 0, flatten([text]))
